...
## Document the lazy-lock.json commit habit, fix stale pylsp mention

`lazy-lock.json` is generated by `:Lazy update` but only does its job (making
installs reproducible) if the resulting diff is committed right away — this
slipped once before (see "Regenerate the lock file" below: `codecompanion.nvim`
and `undotree` were added without their lock entries ever landing). Added a
note to [Installation → Keeping it up to date](installation.md#keeping-it-up-to-date)
spelling this out as a habit, not just a one-off fix.

Also fixed `docs/installation.md`'s step 4, which still told readers to check
`:Mason` for `pylsp` — stale since the switch to `basedpyright`/`ruff` (see
"Switch Python LSP to basedpyright + ruff..." below).
